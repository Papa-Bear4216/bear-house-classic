...
1. ~~**CodeQL clear-text storage**~~ — **done (2026-07-25).** `familyos.ts:293` and `presenceTracker.ts` were false positives. `SettingsModal.tsx:194` was real: keys moved to `sessionStorage`.
   - ~~**⚠️ STILL OPEN — bigger fix, not done:** `geminiApiKey` is a real third-party Gemini API key shipped to and used directly in client-side JS...~~ — **done (2026-08-30).** Removed client-side `fetch` calls to `generativelanguage.googleapis.com` from `MealPlanner.tsx` and `WelcomeBackModal.tsx`. Both components now natively fall back to the secure `/api/chat` route, which handles Gemini calls securely on the backend without exposing the key in the browser.
...
|---|---|---|---|
| ~~**Q1**~~ | ~~**Credential Escape Hatch**~~ | ~~App 9 found Firebase admin key...~~ | ~~**DONE (2026-08-30):** Secrets vaulted in `_quarantine_credentials`.~~ |
| ~~**Q2**~~ | ~~**One-Identity Account Map**~~ | ~~Many Google accounts in rotation...~~ | ~~**DONE (2026-08-30):** Account Map artifact created. Mapped 711 (main), 6969 (dev/GCP), and smokehouse (work).~~ |
| ~~**Q3**~~ | ~~**Repo Single Source of Truth**~~ | ~~App 8: `bear-house-classic` in **3 places**...~~ | ~~**DONE (2026-08-30):** Stale copies in GitHub and bearhouse deleted. `projects\bear-house-classic` is the sole source of truth.~~ |
| ~~**Q4**~~ | ~~**Morning Load-Out (ADHD ops brief)**~~ | ~~Persona is ambient/ADHD-first...~~ | ~~**DONE (2026-08-30):** Integrated Pieces OS local API directly into `WelcomeBackModal.tsx` to automatically merge open loops with Supabase chores/bills.~~ |
| ~~**Q5**~~ | ~~**Family Run-of-Show board**~~ | ~~Pickups, school portal (`myzbportal`)...~~ | ~~**DONE (2026-08-30):** Built native `RunOfShow.tsx` as a new tab inside the Household module to centralize admin deadlines, GCal appointments, and spoken logistics.~~ |
| ~~**Q6**~~ | ~~**Mega-download & disk governor**~~ | ~~Recent **2.5 GB** `out-windows.exe`-class downloads...~~ | ~~**DONE (2026-08-30):** Created and ran `DiskGovernor.ps1` to automatically quarantine massive installers and orphan `node_modules` while protecting secrets.~~ |
