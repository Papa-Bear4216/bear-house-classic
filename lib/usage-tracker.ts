...
export interface HermesMemory {
  ...
  persistentNotes: string[]; // saved memory entries from Hermes
  lastUpdated: string;
}
...
export async function trackUsage(page: string, action?: string) {
  ...
  try {
    ...
    const memory: HermesMemory = snap.exists() ? snap.data() as HermesMemory : {
      pageFrequency: {}, hourlyActivity: {}, recentQueries: [], commonActions: {}, persistentNotes: [], lastUpdated: '',
    };
...
export async function addHermesMemoryNote(note: string) {
  if (!db) return;
  try {
    const ref = doc(db, 'households', uid(), 'hermesMemory', MEMORY_KEY);
    const snap = await getDoc(ref);
    ...
    memory.persistentNotes = [normalized, ...(memory.persistentNotes ?? [])]
      .filter(Boolean)
      .slice(0, 20);
...
export function buildMemorySummary(memory: HermesMemory): string {
  ...
  const saved = memory.persistentNotes?.slice(0, 3).map((note, index) => `${index + 1}. ${note}`) ?? [];
  const lines = [
    top.length ? `Most used features: ${top.join(', ')}` : '',
    peakHours.length ? `Peak usage patterns: ${peakHours.join('; ')}` : '',
    saved.length ? `Saved memory: ${saved.join(' | ')}` : '',
