...
- Browser code uses the Supabase JS client (`src/lib/sync.ts`) for reads and realtime; `householdAuth.ts` calls `supabase.auth.setSession()` after Google OAuth completes, so this shared client actually carries a real `authenticated` session, not `anon`. RLS (see `supabase/migrations/20260714064504_tighten_family_data_rls.sql`) scopes `family_data` selects to rows where `household_id` matches the caller's own `household_members` row via `auth.uid()` — this is real database-enforced isolation for the browser read path, independent of `resolveHouseholdId`. It does not extend to `api/` reads (see above): those always use service_role, so RLS never applies there. No client ever writes with anon — all writes are proxied through `api/` Edge Functions using service_role.
