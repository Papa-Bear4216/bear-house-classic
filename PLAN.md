...
**2026-08-04 follow-up**: The 2026-07-24 pass lazy-loaded every AppLayout module rendered by user navigation, but missed 3 components that are *always or conditionally mounted alongside* the current view rather than switched to by it: `HermesChat` (749 lines, always-rendered floating chat widget), `QuickCapture` (242 lines, always-rendered), `WelcomeBackModal` (151 lines, conditionally rendered on `autobrief`). All three were still statically imported in `AppLayout.tsx`, forcing their code into the main chunk on every cold start regardless of whether the user ever opens chat. Converted all three to `React.lazy()` + wrapped their render site in the same `<Suspense fallback={null}>` pattern already used for `SettingsModal`/`HistoryModal` a few lines above. Also confirmed `marked` and `highlight.js` (both in `package.json` dependencies) have zero imports anywhere in `src/` or `api/` — dead weight from an earlier markdown-rendering approach, since replaced — and removed both via `npm uninstall`. **Result: main chunk 760.31 kB → 732.77 kB gzipped 228.94 kB → 215.85 kB.** Smaller win than the July pass (these 3 components are lighter than the 16 navigation modules), but the shape of the fix — eager imports of code that isn't the initial view — was the exact same class of bug the July pass's own stated rule was meant to prevent; these three just fell outside its literal scope (AppLayout's `switch`/modal-`open` set) despite matching its intent. Verified: `npx tsc --noEmit -p .` clean, `npm test` (179 tests), `npm run lint`, `npm run build` all pass. Same caveat as before — no browser extension connected this session, so no live click-through confirming the chat widget still opens correctly post-lazy-load; the `Suspense` pattern is identical to the already-verified `SettingsModal`/`HistoryModal` case, so risk is low, but call this unverified-in-browser, not confirmed.
**Files**: `src/components/AppLayout.tsx`, `src/components/familyos/Dashboard.tsx`, new `src/components/familyos/sections/carMaintenanceKeys.ts` and `mealPlannerShared.ts`, plus `CarMaintenance.tsx`/`MealPlanner.tsx`/`HermesChat.tsx`/`hermesActions.ts` updated to use them, `package.json` (marked/highlight.js removed 2026-08-04)
**Estimate**: 3 days estimated; actual: same session for the code-splitting piece. Image optimization still open.
