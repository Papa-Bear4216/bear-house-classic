  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowAddCar(f => !f)} className="flex items-center gap-1 bg-honey-500 hover:bg-honey-400 text-white text-sm px-3 py-1.5 rounded-lg transition focus-ring">
        ...
        <div className="bg-bark-700/60 border border-cream-400/10 rounded-xl p-4 space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <div className="col-span-2">
              <label className="text-cream-400/60 text-xs uppercase tracking-wide mb-1 block">Nickname (e.g. "The Truck")</label>
              <input value={carName} onChange={e => setCarName(e.target.value)} placeholder="Car nickname" className="w-full bg-bark-800 border border-cream-400/10 rounded-lg px-3 py-2 text-white text-sm placeholder-cream-400/50 focus:border-honey-500 outline-none focus-ring" autoFocus />
            </div>
            <div>
              <label className="text-cream-400/60 text-xs uppercase tracking-wide mb-1 block">Year</label>
              <input value={carYear} onChange={e => setCarYear(e.target.value)} placeholder="2022" className="w-full bg-bark-800 border border-cream-400/10 rounded-lg px-3 py-2 text-white text-sm placeholder-cream-400/50 focus:border-honey-500 outline-none focus-ring" />
            </div>
            <div>
              <label className="text-cream-400/60 text-xs uppercase tracking-wide mb-1 block">Make</label>
              <input value={carMake} onChange={e => setCarMake(e.target.value)} placeholder="Toyota" className="w-full bg-bark-800 border border-cream-400/10 rounded-lg px-3 py-2 text-white text-sm placeholder-cream-400/50 focus:border-honey-500 outline-none focus-ring" />
            </div>
            <div className="col-span-2">
              <label className="text-cream-400/60 text-xs uppercase tracking-wide mb-1 block">Model</label>
              <input value={carModel} onChange={e => setCarModel(e.target.value)} placeholder="Tacoma" className="w-full bg-bark-800 border border-cream-400/10 rounded-lg px-3 py-2 text-white text-sm placeholder-cream-400/50 focus:border-honey-500 outline-none focus-ring" />
            </div>
          </div>
          <div className="flex gap-2 justify-end">
            <button onClick={() => setShowAddCar(false)} className="text-cream-400/60 hover:text-white text-sm px-3 py-1.5 rounded transition focus-ring">Cancel</button>
            <button onClick={addCar} className="bg-honey-500 hover:bg-honey-400 text-white text-sm px-4 py-1.5 rounded-lg transition focus-ring">Add Car</button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        {activeCars.map(car => {
          ...
          return (
            <div key={car.id} className="bg-bark-700/40 border border-cream-400/10 rounded-xl overflow-hidden">
              <div
              ...
              >
                <div className="w-9 h-9 rounded-lg bg-honey-700/40 border border-honey-500/30 flex items-center justify-center flex-shrink-0">
                  <Car className="w-5 h-5 text-honey-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-white font-medium">{car.name}</div>
                  <div className="text-cream-400/60 text-xs">{[car.year, car.make, car.model].filter(Boolean).join(' ')} · {car.entries.length} records</div>
                  {lastOil && <div className="text-cream-400/50 text-xs">Last oil change: {lastOil.date}{lastOil.mileage ? ` @ ${lastOil.mileage} mi` : ''}</div>}
                </div>
                <div className="flex items-center gap-2">
                  {isAdm && (
                    <button onClick={(e) => { e.stopPropagation(); deleteCar(car.id); }} className="text-cream-400/60 hover:text-rose-400 transition focus-ring">
              ...
              {isExpanded && (
                <div className="border-t border-cream-400/10 px-4 py-3 space-y-3">
                  ...
                  {addEntryFor === car.id && (
                    <div className="bg-bark-800/60 border border-cream-400/10 rounded-lg p-3 space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-cream-400/60 text-xs mb-1 block">Service type</label>
                          <select value={entryType} onChange={e => setEntryType(e.target.value)} className="w-full bg-bark-700 border border-cream-400/10 rounded px-2 py-1.5 text-white text-xs outline-none focus-ring">
                            {MAINTENANCE_TYPES.map(t => <option key={t}>{t}</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="text-cream-400/60 text-xs mb-1 block">Date</label>
                          <input type="date" value={entryDate} onChange={e => setEntryDate(e.target.value)} className="w-full bg-bark-700 border border-cream-400/10 rounded px-2 py-1.5 text-white text-xs outline-none focus-ring" />
                        </div>
                        <div>
                          <label className="text-cream-400/60 text-xs mb-1 block">Mileage</label>
                          <input value={entryMileage} onChange={e => setEntryMileage(e.target.value)} placeholder="e.g. 45000" className="w-full bg-bark-700 border border-cream-400/10 rounded px-2 py-1.5 text-white text-xs placeholder-cream-400/50 outline-none focus-ring" />
                        </div>
                        <div>
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <button onClick={() => addEntry(car.id)} className="bg-honey-500 hover:bg-honey-400 text-white text-xs px-3 py-1 rounded transition focus-ring">Save</button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    {car.entries.map(entry => (
                      <div key={entry.id} className="flex items-start gap-3 bg-bark-800/40 rounded-lg px-3 py-2">
                        <div className="flex-1 min-w-0">
