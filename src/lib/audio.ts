// src/lib/audio.ts
// Pure client-side Web Audio API synthesizer for retro chimes and celebrations
...
class AudioSynth {
  ...
  playLevelUp() {
    ...
    this.playTone(392.0, 'triangle', now + 0.16, 0.1); // G4
    this.playTone(523.25, 'sine', now + 0.24, 0.3); // C5
  }

  // Play a timer/collision alert sound
  playTimerAlert() {
    this.init();
    if (!this.ctx) return;
    if (this.ctx.state === 'suspended') {
